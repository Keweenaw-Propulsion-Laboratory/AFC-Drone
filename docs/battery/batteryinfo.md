...
The voltage getter reports a cached exponential rolling average with a 16-sample
equivalent weight, updated once per main-loop pass.
...
* **Fault Tolerance:** Readings below approximately 12.76V or above 16.91V
	are treated as `FAULT_ERROR`, using a 2.5% margin around the mapped voltage
	limits. Normal Bounds are from 13.09-16.5V.
...
* **Fault Handling:** Voltage readings more than 2.5% below 13.09V or above
	16.50V return `FAULT_ERROR` to flag hardware disconnects or invalid sensor
	readings. Readings within that tolerance remain `CRITICAL_BATTERY` or
