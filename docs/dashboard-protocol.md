...
| 18 | `uint16` | `voltage` | Battery voltage | Smoothed pack voltage from `Battery::getVoltage()`, fixed-point ×1000 (V → mV). Divide by 1000 to display volts. Clamped to 0–65535, so the field cannot wrap on a bad reading. Reads `0` while `BATTERY_PIN` is unassigned. See [Battery Monitor](battery/batteryinfo.md) |
